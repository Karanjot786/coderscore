...
const Navbar = ({ Logout,user, cart, addToCart, removeFromCart, clearCart, subTotal }) => {
    ...
    return (
        <div>
            ...
            <div ref={ref} className={`w-72 h-full sideCart overflow-y-scroll absolute top-0 right-0 bg-pink-300 p-10 transform transition-transform ${Object.keys(cart).length !== 0 ? 'translate-x-0' : 'translate-x-full'} `}>
                <h2 className='font-bold text-2xl'>Shopping Cart</h2>
                <span onClick={toggleCart} className='absolute top-5 right-2 cursor-pointer text-2xl text-pink-900'><AiFillCloseCircle /></span>
                <ol className='list-decimal font-semibold'>
                    {Object.keys(cart).length == 0 && <div className='my-4 font-semibold'> no items</div>}
                    {Object.keys(cart).map((k) => {
                        return <li key={k}>
                            <div className='item flex my-5'>
                                <div className='w-2/3 font-semibold'>{cart[k].title}({cart[k].size}/{cart[k].variant})</div>
                                <div className='flex font-semibold items-center justify-center w-1/3 text*lg'> <AiFillMinusCircle onClick={() => { removeFromCart(k, 1, cart[k].price, cart[k].title, cart[k].size, cart[k].variant) }} /><span className='mx-2 '>{cart[k].qty}</span><AiFillPlusCircle onClick={() => { addToCart(k, 1, cart[k].price, cart[k].title, cart[k].size, cart[k].variant) }} /> </div>
                            </div>
                        </li>
                    })}
                </ol>
                <button onClick={clearCart} className="flex mx-auto mt-6 text-white bg-indigo-500 border-0 py-2 px-5 focus:outline-none hover:bg-indigo-600 rounded">clear Cart</button>
